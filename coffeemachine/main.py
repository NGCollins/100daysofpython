...
def remove_resources(resource_key, drink):
...
def take_money(drink):
    ...
    # TODO clean up 4 input lines and replace with iteration through dictionary checking for zero aswell
    quarters_amount = int(input("How many Quarters?")) * COINS["quarters"]
    nickels_amount = int(input("How many Nickels?")) * COINS["nickels"]
    dimes_amount = int(input("How many Dimes?")) * COINS["dimes"]
    pennies_amount = int(input("How many Pennies?")) * COINS["pennies"]
    ...
    if resource_check(drink):
        if money_amount_inserted < drink_cost:
            print(f"Sorry that's not enough money. Money refunded. {money_amount_inserted} need {drink_cost}")
        else:
...
def coffee_machine():
    ...
    while machine_on:
        ...
        users_coffee_choice = input("What would you like? (espresso/latte/cappuccino):")
        if users_coffee_choice in MENU.keys():
            make_drink(users_coffee_choice)
